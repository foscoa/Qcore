...
def get_journal_data(file_path):
    ...
    trade_df['NotionalToBase'] = trade_df['Quantity'].abs() * trade_df['Multiplier'] * trade_df['FXRateToBase'] * \
                                 trade_df['TradePrice']
    ...
    aggregated_positions_df['Symbol'] = aggregated_positions_df['Symbol'].astype(str).apply(lambda x: x[:10] + '...' if len(x) > 10 else x)
    aggregated_positions_df['Description'] = aggregated_positions_df['Description'].astype(str).apply(lambda x: x[:15] + '...' if len(x) > 15 else x)

    # Compute trade duration
    aggregated_positions_df['TradeDuration'] = aggregated_positions_df['LastExitDate'] - aggregated_positions_df['FirstEntryDate']
